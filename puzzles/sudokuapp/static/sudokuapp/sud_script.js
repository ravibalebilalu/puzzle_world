...
// Sudoku styling
function sudokuStyling() {
    const columns = document.querySelectorAll('.col');
    const first_box = [
        0, 1, 2, 6, 7, 8, 9, 10, 11, 15, 16, 17, 18, 19, 20, 24, 25, 26, 54, 30,
        31, 32, 39, 40, 41, 48, 49, 50, 55, 56, 60, 61, 62, 63, 64, 65, 69, 70,
        71, 72, 73, 74, 78, 79, 80,
    ];

    for (let col = 0; col < columns.length; col++) {
        if (first_box.includes(col)) {
            columns[col].style.outline = '4px solid #555';
            columns[col].style.backgroundColor = '#555';
        } else {
            columns[col].style.outline = '4px solid #777';
